...
- `docs/index.html` — the **console** for the example support assistant: three counts
  (need a person / worth a look / handled automatically), the workflow as a strip with
  the changed node marked, a queue, and the flagged path for the selected request.
  This is the product shape; the report is the diagnosis behind it.
