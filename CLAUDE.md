...
Branch `jev-budget-and-trace-persistence`, **13 commits ahead of `main`**, not merged.
217 tests, ruff clean. Total spend across all development so far: ~$0.05.

Merge when ready: `git checkout main && git merge --ff-only jev-budget-and-trace-persistence`
...
repair cycle · JevBench harness · static demo page (`docs/`, GitHub Pages from
`/docs`).
...
~/miniconda3/envs/evaljev/bin/python benchmarks/export_demo_data.py
```
