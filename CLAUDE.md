...
(source: `main` / `/docs`). Three pages, all generated by
`benchmarks/export_demo_data.py` — do not hand-edit the first two:

- `docs/index.html` — the dashboard over all four recorded runs (168 decisions),
  rendered by exactly the code path `evaljev report` runs for a user.
- `docs/incident.html` — the same page narrowed to the `intent` node, whose newest
  window is the `rotate-criteria` fault. Windows pinned to 12 so each is one variant.
...
`jevbench-rotated.jsonl` (12 decisions, $0.0019, recorded 2026-09-21) is the injected
fault: `--degrade rotate-criteria` shifts every option description one position, so
each label keeps its name and inherits its neighbour's meaning. Accuracy 12/12 → 2/12,
schema adherence still 1.000. It is what the incident page is built from.
