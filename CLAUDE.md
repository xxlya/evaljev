...
- `docs/index.html` — the dashboard for **the example support assistant**, not for
  JevBench. A visitor has to see their own kind of system, which a benchmark sweep
  never looks like. 351 decisions, three nodes, four phases.
- `docs/how-it-works.html` — the essay page, reading `docs/data.js` (JevBench).
- `docs/incident.html` — a redirect to `#caught`, kept so an earlier shared link works.
