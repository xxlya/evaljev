...
def test_report_writes_a_self_contained_page(tmp_path, traces_file, capsys):
    out = tmp_path / "report.html"
    assert main(["report", str(traces_file), "-o", str(out)]) == 0
    html = out.read_text()
...
def test_report_can_also_emit_the_json(tmp_path, traces_file):
    ...
    main(["report", str(traces_file), "-o", str(out), "--json", str(blob)])
...
def test_missing_file_fails_with_a_readable_message(tmp_path):
    with pytest.raises(SystemExit) as exc:
        main(["report", str(tmp_path / "nope.jsonl"), "-o", str(tmp_path / "x.html")])
...
def test_empty_file_says_how_to_record_traces(tmp_path):
    ...
    empty.write_text("")
    with pytest.raises(SystemExit) as exc:
        main(["report", str(empty), "-o", str(tmp_path / "x.html")])
