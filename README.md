...
**Live dashboard: <https://xxlya.github.io/evaljev/>** — a three-step support assistant,
351 real decisions, and the question edit that quietly started routing customers to a
human. Point it at your own traces and you get the same page:
...
It opens with what happened, in three lines: **what moved**, **what changed underneath**,
and **what it did to requests** — then the same customer input before and after the change,
side by side. Below that, the path one request took through your decision points, with the
probability behind every step.

Then eight checks, each with the number behind it and the next step:
...
The request path needs one thing from you: pass the same `metadata={"request_id": ...}` at
every `Monitor.run` in a request. Without it the page still works — it just cannot show a
path it was never told about, and says so rather than guessing one.
